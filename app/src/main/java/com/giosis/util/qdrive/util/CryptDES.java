...
public class CryptDES {
    ...
    public static String encrypt(Context appContext, String data) throws Exception {

        if (data == null || data.length() == 0)
            return "";
        try {
            String key = appContext.getString(R.string.des_key);
            if (key.length() > 8) {
            ...
            return Base64.encodeToString(outputBytes1, Base64.DEFAULT);
        } catch (Exception e) {
            throw new Exception("FAIL_ENCRYPT");
        }
    }

    public static String decrypt(Context appContext, String data) throws Exception {
        if (data == null || data.length() == 0)
            return "";
        try {
            String key = appContext.getString(R.string.des_key);
            ...
            byte[] inputBytes1 = Base64.decode(data, Base64.DEFAULT);
