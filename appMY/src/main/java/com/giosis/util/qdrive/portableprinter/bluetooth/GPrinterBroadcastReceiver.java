...
public class GPrinterBroadcastReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action != null && !action.equals(BluetoothDevice.ACTION_FOUND) && !action.equals(BluetoothAdapter.ACTION_DISCOVERY_FINISHED))
            Log.e("print_list", TAG + "  onReceive action : " + action);
            ...
            switch (action) {
                case BluetoothDevice.ACTION_FOUND: { // 불루트스 기기 검색됨.
                    ...
                    if (device != null && device.getBondState() == BluetoothDevice.BOND_BONDED) {
                        ...
                        Log.e("print_list", TAG + "  ACTION_FOUND   " + device.getName() + "  " + device.getAddress() +
                        ...
                        if (BluetoothDeviceData.connectedPrinterAddress.equals(device.getAddress())) {
                            ...
                            Log.e("print_list", TAG + "  Connect Device name " + device.getName() + " / " + device.getAddress());

                            // 프린트 버튼을 눌렀을 때, 디바이스를 찾아서 커넥션이 이루어진다음에  printerConnManagerList 에 소켓 섹션을 담아서 저장한 다음에
                            // SDK 포트로  열린다면 그 커넥션을 저장해 놓고 포트가 열리지 않는다면 커넥션 제거
                            ...
                            if (GPrinterData.printerConnManagerList != null) {
                                ...
                                if (0 < size) {
                                    ...
                                    if (!GPrinterData.printerConnManagerList.get(size - 1).getConnState()) {  // 포트가  열리지 않았다면
                                        ...
                                        Log.e("print_list", TAG + "  not open Port  " + size);
                ...
                case BluetoothAdapter.ACTION_DISCOVERY_FINISHED: {       // 블루투스 기기 검색 종료
                    ...
                    if (GPrinterData.printerConnManagerList != null && 0 < GPrinterData.printerConnManagerList.size()) {
                        Log.e("print_list", TAG + "  FINISHED  size : " + GPrinterData.printerConnManagerList.size());

                        if (GPrinterData.gPrinterHandler != null) {

                            GPrinterData.CONNECTED_PRINT = false;

                            PrinterConnManager connManager = GPrinterData.printerConnManagerList.get(0);
                            Message message = GPrinterData.gPrinterHandler.obtainMessage(GPrinterData.PRINTER_CONNECT_SUCCESS);
                            Bundle bundle = new Bundle();
                            bundle.putString("tracking_no", GPrinterData.TRACKING_NO);
                            bundle.putString("address", connManager.getMacAddress());
                            message.setData(bundle);
                            GPrinterData.gPrinterHandler.sendMessage(message);
                        }
                    } else {

                        Log.e("print_list", TAG + "  ACTION_DISCOVERY_FINISHED  ");
                        if (GPrinterData.gPrinterHandler != null) {
                ...
                case BluetoothDevice.ACTION_ACL_DISCONNECTED: {     // 블루투스 기기 연결 끊어짐.
                    ...
                    if (device != null) {
                        ...
                        if (GPrinterData.printerConnManagerList != null && 0 < GPrinterData.printerConnManagerList.size()) {
                            Log.e("print_list", TAG + "  ACTION_ACL_DISCONNECTED  " + device.getName());
                            ...
                            if (device.getAddress().equals(GPrinterData.printerConnManagerList.get(size - 1).getMacAddress())) {
                                ...
                                Log.e("print_list", TAG + "  DISCONNECTED Size : " + GPrinterData.printerConnManagerList.size());
                                if (GPrinterData.printerConnManagerList.size() == 0) {
                ...
                case GPrinterData.ACTION_CONN_STATE: {  // 블루투스의 연결 상태 변경        // 사용여부 확인
                    ...
                    Log.e("print_list", TAG + "  action_connect_state  state : " + state);
